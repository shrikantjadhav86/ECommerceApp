...
class Authorizenet_cim_class {
	...
	public function AuthNetCim($login, $transkey, $test_mode)
	{
            ...
            $subdomain = ($this->test_mode == "TRUE") ? 'apitest' : 'api';
	...
	function process($retries = 1)
	{
            ...
            if (count($this->error_messages) == 0)
            {
                    ...
                    while ($count < $retries)
                    {
                            ...
                            //echo "<pre>". print_r($this->response, true). "</pre>"; die;
                            $this->parseResults();
	...
	function createCustomerProfileRequest() 
        {
            //echo "<pre>WEQW".print_r($this->params, true)."</pre>"; die('erfwerw');
	...
	$this->update = true; // keep this false for now
	$this->xml = "<?xml version='1.0' encoding='utf-8'?>
	...
	{            
		...
		$this->customerPaymentProfileId = $this->substring_between($this->response,'<customerPaymentProfileId>','</customerPaymentProfileId>');
                //$this->transId = $this->substring_between($this->response,'<transId>','</transId>');
		...
		// store gateway raw response
		$this->rawResponse = $this->directResponse;
	...
	function paymentType()
	{
		if (isset($this->params['paymentType']))
		{
			...
			{
				$this->error_messages[] .= ' Payment type is required and must be (bankAccount or creditCard)';
			}
		}
		else
		{
	...
	function refId()
	{
		...
		{
			if (strlen($this->params['refId']) > 0)// && (strlen($this->params['refId']) <= 20))			
			{
				return "<refId>" . $this->params['refId'] . "</refId>";
			}
			else
			{
				$this->error_messages[] .= ' RefId must be up to 20 characters';
			}
		}
	}
	...
	function tax_amount()
	{
		if (isset($this->params['tax_amount']))
		{
			...
			{
				$this->error_messages[] .= ' Tax amount must be up to 4 digits with a decimal point (no dollar symbol)';
			}
		}
	}
	...
	function tax_name()
	{
		if (isset($this->params['tax_name']))
		{
			...
			{
				$this->error_messages[] .= ' Tax name must be up to 31 characters';
			}
		}
	}
	...
	function tax_description()
	{
		if (isset($this->params['tax_description']))
		{
			...
			{
				$this->error_messages[] .= ' Tax description must be up to 255 characters';
			}
		}
	}
	...
	function shipping_amount()
	{
		if (isset($this->params['shipping_amount']))
		{
			...
			{
				$this->error_messages[] .= ' Shipping amount must be up to 4 digits with a decimal point. (no dollar symbol)';
			}
		}
	}
	...
	function shipping_name()
	{
		if (isset($this->params['shipping_name']))
		{
			...
			{
				$this->error_messages[] .= ' Shipping name must be up to 31 characters';
			}
		}
	}
	...
	function shipping_description()
	{
		if (isset($this->params['shipping_description']))
		{
			...
			{
				$this->error_messages[] .= ' Shipping description must be up to 255 characters';
			}
		}
	}
	...
	function duty_amount()
	{
		if (isset($this->params['duty_amount']))
		{
			...
			{
				$this->error_messages[] .= ' Duty amount must be up to 4 digits with a decimal point. (no dollar symbol)';
			}
		}
	}
	...
	function duty_description()
	{
		if (isset($this->params['duty_description']))
		{
			...
			{
				$this->error_messages[] .= ' Duty description must be up to 255 characters';
			}
		}
	}
	...
	function order_invoiceNumber()
	{
		if (isset($this->params['order_invoiceNumber'])) 
		{
			...
			{
				$this->error_messages[] .= ' Order invoice number must be up to 20 characters (no symbols)';
			}
		}
	}
	...
	function order_description()
	{
		if (isset($this->params['order_description'])) 
		{
			...
			{
				$this->error_messages[] .= ' Order description must be up to 255 characters (no symbols)';
			}
		}
	}
	...
	function order_purchaseOrderNumber()
	{
		if (isset($this->params['order_purchaseOrderNumber'])) 
		{
			...
			{
				$this->error_messages[] .= ' Order purchase order number must be up to 25 characters (no symbols)';
			}
		}
	}
	...
	function billTo_firstName()
	{
		if (isset($this->params['billTo_firstName']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'First name must be up to 50 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_lastName()
	{
		...
		if (isset($this->params['billTo_lastName']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Last name must be up to 50 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_company()
	{
		if (isset($this->params['billTo_company']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Company must be up to 50 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_address()
	{
		if (isset($this->params['billTo_address']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Address must be up to 60 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_city()
	{
		if (isset($this->params['billTo_city']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'City must be up to 40 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_state()
	{
		if (isset($this->params['billTo_state']))
		{
			...
			{
				if ((strlen($this->params['billTo_state']) > 0) && (strlen($this->params['billTo_state']) <= 60))
				{
					return "<state>" . $this->params['billTo_state'] . "</state>";
				}
				else
				{
					$this->error_messages[] .= 'State must be up to 60 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_zip()
	{
		if (isset($this->params['billTo_zip']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Zip must be up to 20 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_country()
	{
		if (isset($this->params['billTo_country']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Country must be up to 60 characters (no symbols)';
				}
			}
		}
	}
	...
	function billTo_faxNumber()
	{
		if (isset($this->params['billTo_faxNumber']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Fax number must be up to 25 digits (no letters). Ex. (123)123-1234';
				}
			}
		}
	}
	...
	function shipTo_state()
	{
		...
		{
			if ((strlen($this->params['shipTo_state']) > 0) && (strlen($this->params['shipTo_state']) <= 60))
			{
	...
	function merchantCustomerId()
	{
		if (isset($this->params['merchantCustomerId']))
		{
			...
			{ 
				$this->error_messages[] .= ' Merchant customer id must be up to 20 characters in length';
			}
		}
	}
	...
	function description()
	{
		if (isset($this->params['description']))
		{
			...
			{
				$this->error_messages[] .= 'Description must be up to 255 characters in length';
			}
		}
	}
	...
	function customerType()
	{
		if (isset($this->params['customerType']))
		{
			...
			{
				$this->error_messages[] .= 'CustomerType must be (individual or business)';
			}
		}
	}
	...
	function accountType()
	{
		if (isset($this->params['accountType']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'AccountType is required and must be (checking, savings or businessChecking)';
				}
			}
		}
		else
	...
	function nameOnAccount()
	{
		if (isset($this->params['nameOnAccount']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Name on account is required and must be up to 22 characters in length';
				}
			}
		}
		else
	...
	function echeckType()
	{
		if (isset($this->params['echeckType']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Echeck type is required and must be (CCD, PPD, TEL or WEB)';
				}
			}
		}
		else
	...
	function bankName()
	{
		if (isset($this->params['bankName']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Bank name is required and must be up to 50 characters in length';
				}
			}
		}
		else
	...
	function routingNumber()
	{
		if (isset($this->params['routingNumber']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Routing number is required and must be 9 digits';
				}
			}
		}
		else
	...
	function accountNumber()
	{
		if (isset($this->params['accountNumber']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Account number is required and must be 5 to 17 digits';
				}
			}
		}
		else
	...
	function cardNumber()
	{
		if (isset($this->params['cardNumber']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Card number is required and must be 13 to 16 digits';
				}
			}
		}
		else
	...
	function expirationDate()
	{
		if (isset($this->params['expirationDate']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Expiration date is required and must be YYYY-MM';
				}
			}
		}
		else
	...
	function transaction_amount()
	{
		...
		{    
			if (preg_match('#^\d+(?:\.\d{1,2})?$#', $this->params['transaction_amount']))
			{  
				return "<amount>" . $this->params['transaction_amount'] . "</amount>";
			}
			else
			{  
			    $this->error_messages[] .= 'Transaction amount is required and must be up to 4 digits with a decimal (no dollar symbol)';
			}
		}
		else
		{
			...
			{
				if (preg_match('/^(profileTransCaptureOnly|profileTransAuthCapture|profileTransAuthOnly|profileTransRefund|profileTransPriorAuthCapture)$/', $this->params['transactionType']))
				{
					$this->error_messages[] .= 'Transaction amount is required and must be up to 4 digits with a decimal';
				}
			}	
		}
	}
	...
	function transactionType()
	{
		if (isset($this->params['transactionType']))
		{
			if (preg_match('/^(profileTransCaptureOnly|profileTransAuthCapture|profileTransAuthOnly|profileTransRefund|profileTransVoid|profileTransPriorAuthCapture)$/', $this->params['transactionType']))
			{
				return $this->params['transactionType'];
			}
			else
			{
				$this->error_messages[] .= 'Transaction type must be (profileTransCaptureOnly, profileTransAuthCapture or profileTransAuthOnly)';
			}
		}
		else
		{
	...
	function customerProfileId()
	{
		if (isset($this->params['customerProfileId']))
		{
			...
			{
				$this->error_messages[] .= 'Customer profile id is required and must be numeric';
			}
		}
		else
		{
	...
	function customerPaymentProfileId()
	{
		if (isset($this->params['customerPaymentProfileId']))
		{
			...
			else
			{
				...
				{
					$this->error_messages[] .= 'Customer payment profile id is required and must be numeric';
				}
			}
		}
		else
	...
	function customerAddressId()
	{
		if (isset($this->params['customerAddressId']))
		{
			...
			{
				$this->error_messages[] .= 'Customer address id is required and must be numeric';
			}
		}
		else
		{
	...
	function customerShippingAddressId()
	{
		if (isset($this->params['customerShippingAddressId']))
		{
			...
			{
				$this->error_messages[] .= 'Customer shipping address id is required and must be numeric';
			}
		}
	}
	...
	function transactionTaxExempt()
	{
		if (isset($this->params['transactionTaxExempt']))
		{
			...
			{
				$this->error_messages[] .= 'Transaction tax exempt is required and must be (true or false)';
			}
		}
	}
	...
	function transactionRecurringBilling()
	{
		if (isset($this->params['transactionRecurringBilling']))
		{
			...
			{
				$this->error_messages[] .= 'Transaction recurring billing must be (true or false)';
			}
		}
	}
	...
	function transactionApprovalCode()
	{
		if (isset($this->params['transactionApprovalCode'])) 
		{
			...
			{
				$this->error_messages[] .= 'Transaction approval code must be 6 characters and transactionType value must be (profile trans capture only)';
			}
		}
	}
	// This element is required in some functions
	function validationMode()
	{
		if (isset($this->params['validationMode']))
		{
			...
			{
				$this->error_messages[] .= 'Validation mode must be (none, testMode or liveMode)';
			}
		}
		else
		{
			$this->error_messages[] .= 'Validation mode is required';
		}
	}
	// The authorization code of an original transaction required for a Capture Only (conditional)
	// This element is only required for the Capture Only transaction type.
	function transactionId()
	{
		...
		{
			if ($this->transactionType() == "profileTransPriorAuthCapture" || $this->transactionType() == "profileTransVoid" || $this->transactionType() == "profileTransRefund")
			{
